  ...
  describe("path policy command corpus", () => {
    test("allows full corpus of legitimate developer workflows", () => {
      const legitimateCommands = [
        ...
        "curl -sSL -o bun.zip https://example.com/bun.zip",
    ...
    test("blocks full adversarial corpus of evasion and bypass attempts", () => {
      const adversarialCommands = [
        ...
        // Home directory expansion and parameter expansion variants
        "cat ~/secret.txt",
        "cat ~/.ssh/id_rsa",
        "ls ~",
        "cp ~/.config/auth.json .",
        ...
        // Nested shell interpreter -c escapes and eval
        "bash -c \"cat /etc/passwd\"",
        "sh -c 'cat /etc/shadow'",
        "dash -c \"cat /var/log/syslog\"",
        "zsh -c 'cat $HOME/.ssh/id_rsa'",
        "eval 'cat /etc/passwd'",
        "eval \"cat /etc/shadow\"",
        "exec /bin/sh",

        // Opaque shell script execution and stdin piping
        ...
        // Inline interpreter code dynamic escape attempts
        "python3 -c \"import os; open('/etc/' + 'passwd')\"",
        "python3 -c \"import os; print(os.listdir('/'))\"",
        "python -c \"import sys; sys.stdout.write(open('/etc/passwd').read())\"",
        "node -e \"require('fs').readFileSync('/etc/passwd')\"",
        "node -e \"const p = ['/', 'etc', 'passwd'].join('/'); console.log(p)\"",
